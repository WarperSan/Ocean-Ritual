...
public static class GemHelper
{
    ...
    public static (int cost, int modifiedSpot, int missingSpot, bool CasseOnlytrue) GetInformationAboutForm(
        List<bool> formOriginal,
        List<bool> modifiedForm,
        int        lvlGem
    )
    {
        ...
        if (modifiedTrueCount > originalTrueCount)
            throw new ArgumentException(
                "Le nombre de 'true' dans la liste modifi�e ne peut pas �tre sup�rieur � celui de la liste originale.");
    ...
    public static int fusionGemTab(int lvlLeftGem, int lvlRightGem) =>
        lvlLeftGem >= lvlRightGem ? lvlLeftGem + (lvlLeftGem == lvlRightGem ? 1 : 0) : lvlLeftGem + 1;

    public static void ModifiedList(
        ref List<bool> ListBool,
        int            Xposition,
        int            YPosition,
        bool           value,
        int            width
    ...
    {
        var list = new List<bool>(gem.form.flatForme);

...

{
    ...
    private static List<int> LuckLevels = new()
    {
        20,
        10,
        5,
    ...
    private static string GemmePath = "Gemme/AllGemme"; // Path to the gems prefabs

    private static string SampleGemmePath = "Gemme/SampleGemme"; // Path to the sample gems prefab

    // static string GemmeName = "Red"; // Default gems name
    //static float Spacebetween = 1f; // Space between gemmes
    private static Dictionary<string, GameObject> DictionaryGemme = new(); // Dictionary to store gems prefabs

    private static GameObject SampleGemme; // Sample gems prefab

    //static int lvlTest = 3; // Test level
    private static int height = 1; // Height of the gems
    ...
    public static List<TypeQuantity<T>> GeneratesRandomlvlStat<T>(int LVL) where T : Enum
    {
        ...
        var resultList = new List<TypeQuantity<T>>();
    ...
    public static string GeneratesRandomlColors()
    {
        ...
        Array colors = Enum.GetValues(typeof(ColorsName));
    ...
    private static FormBool GenerateForme(FormBool gemForm)
    {
        ...
        var falsePositions = new List<int>();
        ...
        else
        {
            ...
            var newForm = new List<bool>(newSize * newSize);
    ...
    public static bool CreateMaterialGemme(GameObject instantiatedGemme)
    {
        ...
        var positionOffset = new Vector3(gemmeScript.PositionX * space, 0, gemmeScript.PositionZ * space);
        ...
        for (int i = 0; i < form.height; i++)
        {
            for (int j = 0; j < form.width; j++)
            {
                if (boolArray[i, j])
                {
                    ...
                    var localPosition = new Vector3((i - centreY) * space, height, (j - centreX) * space);

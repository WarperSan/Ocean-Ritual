...
namespace DhafinFawwaz.AnimationUILib
{
    ...
    {
        [HideInInspector]
        public float TotalDuration; //Value automatically taken care of by AnimationUIInspector
        ...
        private bool _isSequencesInitialized;
        ...
        {
            for (int i = 0; i < atTimeEvents.Count; i++)
                    ...
                    else if (sequence.TargetType == Sequence.ObjectType.TextMeshPro)
                    {
                        ...
                        if (sequence.TargetTextMeshProTask.HasFlag(Sequence.TextMeshProTask.MaxVisibleCharacters))
                        {
                            _coroutines.Add(StartCoroutine(TaskMaxVisibleCharacters(sequence.TargetComp.GetComponent<TMP_Text>(),
                                (float)sequence.MaxVisibleCharactersStart,
                                (float)sequence.MaxVisibleCharactersEnd,
                                sequence.Duration,
                                sequence.EaseFunction
                            )));
                        }
        ...
        private IEnumerator ReverseArrayAtTime(float t)
        {
        ...
        private IEnumerator PlayReversedAnimation()
        {
            Array.Reverse(AnimationSequence);
        ...
        private IEnumerator TaskAnchoredPosition(
            RectTransform rt,
            Vector3       start,
            Vector3       end,
            float         duration,
        ...
        private IEnumerator TaskLocalScale(
            RectTransform rt,
            Vector3       start,
            Vector3       end,
            float         duration,
        ...
        private IEnumerator TaskLocalEulerAngles(
            RectTransform rt,
            Vector3       start,
            Vector3       end,
            float         duration,
        ...
        private IEnumerator TaskAnchorMax(
            RectTransform rt,
            Vector3       start,
            Vector3       end,
            float         duration,
        ...
        private IEnumerator TaskAnchorMin(
            RectTransform rt,
            Vector3       start,
            Vector3       end,
            float         duration,
        ...
        private IEnumerator TaskSizeDelta(
            RectTransform rt,
            Vector3       start,
            Vector3       end,
            float         duration,
        ...
        private IEnumerator TaskLocalPosition(
            Transform     trans,
            Vector3       start,
            Vector3       end,
            float         duration,
        ...
        private IEnumerator TaskFillAmount(
            Image         img,
            float         start,
            float         end,
            float         duration,
        ...
        private IEnumerator TaskBackgroundColor(
            Camera        cam,
            Color         start,
            Color         end,
            float         duration,
        ...
        private IEnumerator TaskOrthographicSize(
            Camera        cam,
            float         start,
            float         end,
            float         duration,
        ...
        private IEnumerator TaskTextMeshProColor(
            TMP_Text      text,
            Color         start,
            Color         end,
            float         duration,
        ...
        private IEnumerator TaskMaxVisibleCharacters(
            TMP_Text      text,
            float         start,
            float         end,
            float         duration,
        ...
        private IEnumerator AtTimeEvent(Action atTimeEvent, float time)
        {
        ...
        private void OnDisable() => UnityEditor.EditorApplication.update -= EditorUpdate;
        ...
        private void OnDrawGizmos() => ForceRepaint();
        ...
        [HideInInspector]
        public float CurrentTime; // Don't forget this variable might be in build

        [HideInInspector]
        public bool IsPlayingInEditMode;
        ...
        private void Reset() //For the default value. A hacky way because the inspector reset the value for Serialized class
        ...
        private void InitFunction() //For preview
        {
            UpdateSequence = null;
                    ...
                    if (sequence.TargetType == Sequence.ObjectType.RectTransform)
                    {
                        ...
                        void RtAnchoredPosition(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                rt.anchoredPosition
                                    = Vector3.LerpUnclamped(sequence.AnchoredPositionStart,
                                        sequence.AnchoredPositionEnd,
                        ...
                        void RtLocalEulerAngles(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                rt.localEulerAngles
                                    = Vector3.LerpUnclamped(sequence.LocalEulerAnglesStart,
                                        sequence.LocalEulerAnglesEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                        ...
                        void RtLocalScale(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                rt.localScale
                                    = Vector3.LerpUnclamped(sequence.LocalScaleStart,
                                        sequence.LocalScaleEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                        ...
                        void RtSizeDelta(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                rt.sizeDelta
                                    = Vector3.LerpUnclamped(sequence.SizeDeltaStart,
                                        sequence.SizeDeltaEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                        ...
                        void RtAnchorMin(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                rt.anchorMin
                                    = Vector3.LerpUnclamped(sequence.AnchorMinStart,
                                        sequence.AnchorMinEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                        ...
                        void RtAnchorMax(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                rt.anchorMax
                                    = Vector3.LerpUnclamped(sequence.AnchorMaxStart,
                                        sequence.AnchorMaxEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                        ...
                        void RtPivot(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                rt.pivot
                                    = Vector3.LerpUnclamped(sequence.PivotStart,
                                        sequence.PivotEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                    ...
                    else if (sequence.TargetType == Sequence.ObjectType.Transform)
                    {
                        ...
                        void TransLocalPosition(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                trans.localPosition
                                    = Vector3.LerpUnclamped(sequence.LocalPositionStart,
                                        sequence.LocalPositionEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                    ...
                    else if (sequence.TargetType == Sequence.ObjectType.Image)
                    {
                        ...
                        void ImgColor(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                img.color
                                    = Color.LerpUnclamped(sequence.ColorStart,
                                        sequence.ColorEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                        ...
                        void ImgFillAmount(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                img.fillAmount
                                    = Mathf.LerpUnclamped(sequence.FillAmountStart,
                                        sequence.FillAmountEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                    ...
                    else if (sequence.TargetType == Sequence.ObjectType.CanvasGroup)
                    {
                        ...
                        void CgAlpha(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                cg.alpha
                                    = Mathf.LerpUnclamped(sequence.AlphaStart,
                                        sequence.AlphaEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                    ...
                    else if (sequence.TargetType == Sequence.ObjectType.Camera)
                    {
                        ...
                        void CamBackgroundColor(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                cam.backgroundColor
                                    = Color.LerpUnclamped(sequence.BackgroundColorStart,
                                        sequence.BackgroundColorEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                        ...
                        void CamOrthographicSize(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                cam.orthographicSize
                                    = Mathf.LerpUnclamped(sequence.OrthographicSizeStart,
                                        sequence.OrthographicSizeEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                    ...
                    else if (sequence.TargetType == Sequence.ObjectType.TextMeshPro)
                    {
                        ...
                        void TextMeshProColor(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                text.color
                                    = Color.LerpUnclamped(sequence.TextMeshProColorStart,
                                        sequence.TextMeshProColorEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                        ...
                        void MaxVisibleCharacters(float t)
                        {
                            if (0 <= t - sequence.StartTime && t - sequence.StartTime < sequence.Duration)
                            {
                                ...
                                text.maxVisibleCharacters
                                    = (int)Mathf.LerpUnclamped(sequence.MaxVisibleCharactersStart,
                                        sequence.MaxVisibleCharactersEnd,
                                        sequence.EaseFunction(Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }

...
public class ObjectPool : MonoBehaviour
{
    ...
    public GameObject GetObjectRandomRayon(GameObject typeObject, Vector3 randomCirclePos, GameObject parent)
    {
        Debug.Log("plantedans objectpool");
        for (int i = 0; i < pool.Count; i++)
        {
            
            if (pool[i].name == typeObject.name && !pool[i].activeInHierarchy) return pool[i];
        }
        GameObject obj = Instantiate(typeObject, randomCirclePos, Quaternion.identity, parent.transform);
        obj.name = typeObject.name;
        obj.SetActive(false);
        pool.Add(obj);
        return obj;
